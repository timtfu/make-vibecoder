...
server.registerTool('tools_documentation', {
    ...
    const doc = {
        ...
        tools: {
            ...
            list_scenarios: 'List all existing scenarios in your Make.com account. Returns scenario names, IDs, scheduling types, and status. Useful for managing scenarios.',
        ...
        tips: [
            ...
            'Your Make API key must have "scenarios:read" and "scenarios:run" scopes for full functionality',
        ],
    };
    ...
    function checkValue(value: any, path: string) {
        if (typeof value === 'string' && value.includes('{{')) {
            ...
            for (const match of matches) {
                ...
                } else if (refModuleIndex > moduleIndex) {
                    warnings.push(
                        `${pos} (${flowModule.module}): Expression {{${refModuleIndex}.${field}}} in "${path}" ` +
                        `references future module ${refModuleIndex}. Current module is at index ${moduleIndex}.`
                    );
...
function validateDependencies(
    ...
    for (let i = 0; i < flow.length; i++) {
        ...
        for (const refIndex of references) {
            if (refIndex >= i) {
                errors.push(
                    `${pos}[${i}] (${flowModule.module}): Module references {{${refIndex}.*}} but module ${refIndex} ` +
                    `comes after current module ${i}. Forward references are not allowed in Make.com.`
                );
            }
        }
        ...
        if (references.has(i)) {
            errors.push(
                `${pos}[${i}] (${flowModule.module}): Module references itself {{${i}.*}}. Self-references are not allowed.`
            );
        }
    }
}
